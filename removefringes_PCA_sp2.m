...
filename_chopped = frName_sp2(2:underscoreIndex(2));

A_reshaped = make_edge_vector(As,pp1,pp2);
num_images = length(refindex_sp2)+1;  % number of reference images
...
for ii = 2:1:num_images
    jj = jj + 1;
    filename_ref = [filename_chopped num2str(refindex_sp2(ii-1)) '.asc'];
    refimgb = loadascii([frPath_sp2 'b' filename_ref]);
    refimgb = rot90(refimgb,numrotations_sp2);
    refimgc = loadascii([frPath_sp2 'c' filename_ref]);
